...
export function LabelModal({ isOpen, onClose }: LabelModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('[LABEL_MODAL_ERROR]', err);
      toast.error('Initialization failed: System error encountered.');
    }
  };
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-sm border border-border bg-card z-101 shadow-2xl overflow-hidden"
          >
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-sm bg-primary/10">
                    <Icon icon="solar:tag-linear" className="w-5 h-5 text-primary" />
                  </div>
