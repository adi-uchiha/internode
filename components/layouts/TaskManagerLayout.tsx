...
const adminNavItems: { section: string; items: NavItemDef[] }[] = [
  ...
  {
    section: 'TEAM', items: [
      { label: 'Members', href: '/tasks/members', icon: 'solar:users-group-rounded-linear', adminOnly: true },
      { label: 'Analytics', href: '/tasks/analytics', icon: 'solar:graph-up-linear', adminOnly: true },
    ]
  },
  {
    section: 'SYSTEM', items: [
      { label: 'Settings', href: '/tasks/settings', icon: 'solar:settings-linear', adminOnly: true },
    ]
  },
];
                  ...
                  {visibleItems.map((item) => {
                    ...
                    return (
                      <Link
                        ...
                        href={item.href}
                        className={cn(
                          "flex items-center gap-3 px-3 py-2 transition-all duration-200 relative",
                          isActive
                      ...
                      >
                        ...
                        <Icon icon={item.icon} className={cn("w-[18px] h-[18px] shrink-0", isActive && "text-primary")} />
                        <AnimatePresence>
                          {!collapsed && (
                            <motion.span
                              initial={{ opacity: 0 }}
      ...
      <main className="flex-1 transition-all duration-300" style={{ marginLeft: collapsed ? 64 : 240 }}>
        ...
        <header className="h-12 border-b border-border bg-card/50 backdrop-blur-sm flex items-center justify-between px-6 sticky top-0 z-40">
          <div className="flex items-center gap-4">
            <button onClick={() => setCollapsed(!collapsed)} className="p-1 hover:bg-muted/50 transition-colors">
          ...
          >
            ...
            <span className="font-mono text-xs flex-1 text-left">&gt; search tickets, members...</span>
                ...
                {showNotifications && (
                  ...
                  >
                    <div className="flex items-center justify-between p-4 border-b border-border">
                      ...
                      <button className="font-mono text-xs text-primary hover:underline">Mark all</button>
                    </div>
                    <div className="overflow-y-auto max-h-[400px]">
                      {tmNotifications.map(n => (
                        ...
                        >
                          ...
                          <div className="font-mono text-xs text-muted-foreground ml-4">{n.subtitle}</div>
                          <div className="font-mono text-[10px] text-muted-foreground ml-4 mt-1">{n.time}</div>
                        </div>
                      ))}
                    </div>
                    <div className="p-3 text-center border-t border-border">
                      <button className="font-mono text-xs text-primary hover:underline">View all notifications →</button>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
        ...
        {showSearch && (
          <>
            ...
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -20 }}
              className="fixed top-[20%] left-1/2 -translate-x-1/2 w-[600px] max-w-[90vw] border border-border bg-card z-101"
            >
              ...
              <div className="p-4">
                <div className="font-mono text-[10px] text-muted-foreground uppercase tracking-widest mb-3">RECENT</div>
                {[
                  { icon: 'solar:document-text-linear', title: 'Fix auth redirect loop', sub: 'TICKET-42' },
                  { icon: 'solar:document-text-linear', title: 'Update API docs', sub: 'TICKET-41' },
                ].map((item, i) => (
                  <div key={i} className="flex items-center gap-3 p-2 hover:bg-muted/30 cursor-pointer transition-colors" onClick={() => setShowSearch(false)}>
                ...
                <div className="font-mono text-[10px] text-muted-foreground uppercase tracking-widest mt-4 mb-3">QUICK ACTIONS</div>
                {[
                ...
                ].filter(a => !a.admin || isAdmin).map((item, i) => (
                  <div key={i} className="flex items-center gap-3 p-2 hover:bg-muted/30 cursor-pointer transition-colors" onClick={() => setShowSearch(false)}>
